...
- `turbopack.resolveAlias['#bookmark-local']` — resolves to `src/lib/bookmarking/local.ts` when that file exists on disk (checked with `fs.existsSync` at config-load time), and to the tracked empty slot `src/lib/bookmarking/localNone.ts` otherwise. Lets a deployment override the bookmark naming scheme with one untracked file, while every other clone builds unchanged. `vitest.config.ts` and `tsconfig.json` alias the same specifier for tests and type-checking.
