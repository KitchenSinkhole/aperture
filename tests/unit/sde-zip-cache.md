...
- Runs against the real `.sde-cache/` directory under build number `999000001`, which CCP will never publish; every `sde-999000001-jsonl.zip*` entry is removed before and after each case.
...
- A complete download lands at `.sde-cache/sde-<build>-jsonl.zip` at full length, and a second call serves it from disk without a second `fetch`.
