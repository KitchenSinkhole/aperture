...
### Depends On
- `next`, `node:http`, `@next/env` (`loadEnvConfig`), `@/lib/realtime/wsServer` (`attachWsServer`, dynamically imported), `@/lib/jobs/runner` (`startWorker` / `stopWorker`, dynamically imported), `@/lib/integrations/zkbFeed` (`startZkbFeed` / `stopZkbFeed`, dynamically imported), `@/lib/env`.
