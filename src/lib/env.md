...
- `INTEGRATIONS_ENABLED` — opt-in switch for the `/api/integrations/*` route group (machine-to-machine token-authenticated data feeds). String `'true'`/`'false'`, parsed to a boolean; defaults to `false`. When `false` every route in the group returns 404. Per-request auth is a per-token `ap_integration_token` row, not this flag — this only gates whether the group exists.
