...
Pure splitter — no DB, no `Date.now()`. The EVE D-Scan emits 4 tab-separated columns in fixed order: `TypeID, Name, Type, Distance`. A row is accepted only when all four columns are present, cell 0 is digits-only, and cells 1 and 2 are non-blank — a language-independent gate that rejects ordinary typed text, so an empty result is a reliable "this paste was not a D-Scan" signal for callers. The full-width requirement matters because a caller that recognizes a paste swallows it: three-column tabular text with a numeric first cell would otherwise cost the user their query. Name and Type are read from fixed positions at the tail — Type is the second-to-last cell, Name everything between the id and it — so a name holding its own run of two or more spaces, which over-splits under the no-tabs fallback, still yields the right Type; the name's run is rejoined at two spaces. Falls back to splitting on 2+ spaces for clipboards that strip tabs. Distance has to be present for the row to count, but its content is never read and is not carried in the output (it reads `-` for objects outside the range readout).
