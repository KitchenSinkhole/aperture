...
Inserts one `ap_map_connection` row between two map systems. Validates both `sourceMapSystemId` and `targetMapSystemId` belong to `mapId` via `assertSystemOnMap` (`./tenancy`) before the insert — throws and rolls back on a foreign id; asserting both endpoints against the same `mapId` also guarantees the new edge cannot span two maps. Flag defaults: `massStatus = 'fresh'`, `jumpMassClass = null`, `eolStage = 'none'`, booleans `false`; `eol_at` is stamped only when the connection is created at a non-`none` `eolStage`. `confirmed_at` is always set to `now()` — every create (manual draw, sig link, stargate auto-link) is a fresh observation, so the row is born confirmed (dormancy is only applied later by `removeSystem` NULLing incident `wh` rows). Emits `connection.create` with the full edge body (id/source/target/scope/massStatus/jumpMassClass/eolStage + flags). `confirmed_at` is not in the payload — the client never reads it; dormant rows are simply never sent.
