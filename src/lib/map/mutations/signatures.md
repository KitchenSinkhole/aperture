...
The event also carries a `snapshot` — the **full post-update row** (every `signatureBody` field, `leadsToMapSystemId` omitted as the canvas ignores it) — so a client whose baseline for this sig is missing/wrong (reconnect gap, missed `signature.create`, reordering) can upsert the whole row instead of silently no-op'ing the merge-by-id. It is purely **additive**: the conditional audit fields and Discord/audit no-op suppression read only the changed-key fields and ignore `snapshot`, so precision is untouched. Marginal cost is one `universe_wormhole` lookup whenever the post-update row has a non-null `typeId` (resolved once, reused by both the `wormholeCode` audit field and the snapshot).
