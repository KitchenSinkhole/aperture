...
**Bubbled ends:** `sourceBubbled` / `targetBubbled` are forced to `false` unless `profile.showBubbles` is set, so a suppressed flag never reaches the client. Both keys are always present on the edge — the redaction is the value, not the shape.
