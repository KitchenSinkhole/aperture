...
### Types
- `PublicMapSystemNode` — `{ id, systemId, name, tag, status, security, trueSec, effect, regionName, constellationName, statics, tradeHub, positionX, positionY }`. Has no `alias`, `intelNotes`, lock fields, or `rallyAt` — those simply are not fields on this type.
...
- `PublicMapViewData` — `{ map: { name, shareLabel }, systems, connections, signatures, presence }`. No `id`, `scope`, `type`, `tagScheme`, `homeMapSystemId`, `notes`, `intel`, `structures`, or stats field exists on this type.

### Depends on
- `@/db/client` (`db`), `@/db/schema` (tables + enums), `@/lib/map/share` (`resolveShareToken`), `@/lib/map/loadMap` (`loadStatics`, `loadMapPresence`).
