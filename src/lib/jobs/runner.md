...
Idempotent boot. Runs graphile-worker's own `runMigrations` to create/upgrade the `graphile_worker` schema, re-arms the location-poll loop after an unclean shutdown, reaps location-poll zombies, closes orphaned `ap_job_run` rows (all three below), then calls `run({ pgPool, taskList, parsedCronItems, … })` and returns the `Runner`. Repeat calls in the same process return the existing instance.
...
- **Orphaned job-run close (all environments).** Last of the three boot sweeps, `startWorker` sets `ended_at = now()`, `success = false` and an explanatory `error_text` on every `ap_job_run` row with `ended_at IS NULL`. `withInstrumentation` ([[withInstrumentation]]) finalises its row from a `catch`, so an un-ended row means the process was killed outright — SIGKILL on a container recreate, a host OOM kill, a V8 abort — and nothing else ever ends it. The `job_abandoned` alert rule ([[rules]]) counts exactly this shape, so without the sweep one killed handler leaves the instance alerting permanently with no path back to healthy. Boot is the safe moment because no handler of this process is in flight yet; a run still executing in an overlapping instance is re-finalised correctly by its own handler's terminal `UPDATE`. Close count is logged at `warn`.
