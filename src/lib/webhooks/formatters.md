...
**Feature delegation** is named directly from the payload (no `ctx` lookup): `access.granted` → `"granted title **<title>** audit-log access"`, `access.revoked` → `"revoked title **<title>**'s webhook management"`, mapping each `map_capability` value to a human phrase via `CAPABILITY_LABEL`.
