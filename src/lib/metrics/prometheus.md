...
Gauge names/help live in a module-private `GAUGE_METRICS` table that maps each flat `GaugeReadings` key to a conventionally-named Prometheus gauge (`*_bytes`, `*_ms`, etc.): `tracked_characters`, `visible_systems`, `ws_connections`, `esi_breakers_open`, `job_backlog`, `jobs_abandoned`, `process_resident_memory_bytes`, `process_heap_used_bytes`, `process_heap_total_bytes`, `event_loop_lag_ms`.
