...
### Props
Receives xyflow `EdgeProps` with `data: PublicConnectionEdgeData` (`PublicMapConnectionEdge` plus `endpointSecurity: { source, target }` — each end's `universe_system.security` label, for tinting the sig tags — and `onHoverChange: (connectionId: string | null) => void`, which this edge calls on hover/unhover so `SpectatorMap` can ring both endpoint tiles).
