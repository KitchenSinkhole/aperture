...
### Local State
- `pending: { characterId, fromSystemId, toSystemId } | null` — the latest own-pilot jump not yet resolvable against live props.
- `snapshot: { here, cameFrom, connection, hopsFromHome, homeMapSystemId } | null` — the frozen transit context the displayed pair is derived from; `null` before the session's first qualifying transit.
