...
**Purpose:** Standalone full-width signatures panel rendered below the map. Renders a filterable, sortable nine-column table (class-kind icon / Sig / Group / Type / Description / Leads to / Created / Updated / delete) for the selected system; placeholder when nothing is selected.
...
- **Zebra striping:** body rows alternate background (even rows tinted) to aid scanning down the list.
...
- The filter bar also shows unfiltered system-wide stats beside the scan-state button — total signature count and unscanned count always show (e.g. "12 signatures · 4 unscanned"); the unscanned count excludes only anomalies (`classKind === 'anomaly'`) — manual/unknown-kind sigs (`classKind === null`) count as unscanned alongside cosmic signatures. A wormhole count segment (e.g. "· 3 wormholes") appends only when the system has at least one wormhole sig — unaffected by the active group/scan filters.
