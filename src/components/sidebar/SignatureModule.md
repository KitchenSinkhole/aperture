...
**Purpose:** Standalone full-width signatures panel rendered below the map. Renders a filterable, sortable ten-column table (class-kind icon / Sig / Group / Type / Description / Leads to / TTL / Created / Updated / delete) for the selected system; placeholder when nothing is selected.
...
- Body only: when no system is selected, a placeholder message. When a system is selected: a **filter bar** (`SignatureFilterBar`) above the table, the ten-column TanStack Table, and a draft-input row below. TTL is a forward countdown (`formatRelativeFromMs`); Created and Updated are backward "time ago" strings (`formatAgoFromMs`). The panel name ("Signatures"), drag handle and hide button come from the surrounding `MapPanel` chrome.
...
- The **Sig**, **Group**, **Created**, and **Updated** column headers are sortable (click to sort ascending, click again to reverse, arrow indicator shows active sort). Default sort is Sig ascending. Other columns (class-kind icon, Type, Description, Leads to, TTL, delete) are non-interactive headers.
...
- **Stable cell identities (don't close open selects on realtime updates):** the ten table cells are rendered by **module-level** components (`ClassKindCell`, `SigIdCell`, `GroupCell`, `TypeColumnCell`, `DescriptionCell`, `LeadsToCell`, `TtlCell`, `CreatedCell`, `UpdatedCell`, `ActionsCell`) referenced from a single module-level `signatureColumns` array — **not** an in-`useMemo` columns array. This is load-bearing: TanStack's `flexRender` renders each `columnDef.cell` as a React component *type*, so when the cell was a fresh closure per render (the old approach) React saw a new type at every cell on each data change and unmounted + remounted the whole cell subtree. That remount slammed an open in-row `Select` (e.g. WH type, Group, Leads-to) shut the instant another viewer edited a signature in the same system — the system-scoped edit churns `rows` → `assignedConnectionIds` → the columns memo. The volatile data the cells need (`system`, `connections`, `systems`, `onPatch`, `onDelete`, `syncConnectionSize`, `metaByTypeId`, `assignedConnectionIds`) is passed in through TanStack's `table.options.meta` (typed `SignatureTableMeta`) and read via `table.options.meta as SignatureTableMeta` inside each cell, so the cell function identities never change and a realtime update re-renders cells **in place** rather than remounting them. The Base UI `Select` keeps its open state internally (uncontrolled), so in-place re-render leaves an open dropdown open.
...
| system | MapSystemNode \| null | yes | The selected system; when `null` no actions are rendered (lazy-delete and paste are gated on a selection). |
