...
- The filter bar also shows unfiltered system-wide stats beside the scan-state button — total signature count and unscanned count always show (e.g. "12 signatures · 4 unscanned"); the unscanned count only considers `classKind === 'signature'` rows (anomalies never count as unscanned here). A wormhole count segment (e.g. "· 3 wormholes") appends only when the system has at least one wormhole sig — unaffected by the active group/scan filters.
