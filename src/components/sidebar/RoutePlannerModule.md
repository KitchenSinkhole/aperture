...
- **Recompute** — debounced (300ms) `POST /api/map/[mapId]/route-plan` whenever the source, prefs, destinations, a connection signature (`id:scope:mass:eol:jumpMass`), or a connection-bound scan sig (`connectionId:mapSystemId:sigId`, sorted) changes — the latter because the instructions name the sig flown through, so scanning or correcting one has to re-plan; out-of-order responses are dropped via a seq ref; results stored locally.
...
- **Stale plan while recomputing** — `plans` is cleared only when there is no source or no destinations, so across the debounce and the in-flight request a destination still holds its **previous** plan: the steps and **Copy** describe the route as it was before the change until the response lands.
