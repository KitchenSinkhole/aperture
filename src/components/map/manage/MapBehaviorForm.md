...
- Submits all three toggles via `updateMapSettingsAction` (gated by `canManageMap`); toasts success/error, and calls `router.refresh()` on success so the reopened dialog reflects the saved values.
