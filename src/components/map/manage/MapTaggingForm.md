...
- Submits `tagScheme` + `homeMapSystemId` + `exemptHomeStaticFromTag` via `updateMapSettingsAction` (gated by `settings_manage`); calls `router.refresh()` on success so the reopened dialog reflects the saved values.
