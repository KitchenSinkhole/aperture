...
|---|---|---|---|
| presence | readonly PresenceTableRow[] | yes | The pilots in one system. Sorted by character name; rendered as-is (no re-filtering). |
...
### Exports
- `PresenceTableRow` — `{ characterId, characterName, shipClass, shipTypeName, shipName }`, the fields the table actually reads. Both `MapPresenceEntry` and the public share's `PublicPresencePilot` satisfy it.

### Depends On
- `customShipName` from `@/lib/map/shipName` — the pilot's custom hull name, or `''` when un-renamed.
- `ShipClassIcon` from `@/components/icons/ShipClassIcon`
- Type `ShipClass` from `@/types`.
