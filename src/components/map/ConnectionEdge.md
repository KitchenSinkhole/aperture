...
- Edits all live in the sidebar inspector (`InspectorModule.ConnectionInspector`), except the per-end bubbled flag, which is edited from the endpoint's own context menu.
...
- **Endpoint hover:** the wrapping `<g>` tracks pointer enter/leave and passes `visible` to both `ConnectionEndpoint`s, so each end's dot only shows while the cursor is over the edge (line or endpoint hit circle — both are descendants of the same `<g>`, so moving between them doesn't drop hover). Right-clicking an endpoint calls `data.onEndpointContextMenu` with that end and stops the event from also reaching xyflow's edge-level `onEdgeContextMenu`; left-clicking one is left to bubble so the existing edge-click selection still fires.
- **Bubbled ends:** a `ConnectionBubble` renders per flagged end (`sourceBubbled`/`targetBubbled`), independent of hover — the bubble is a persistent state marker, the endpoint dot a transient hover affordance.
...
- `./ConnectionBubble` — the bubbled-end marker and wash.
