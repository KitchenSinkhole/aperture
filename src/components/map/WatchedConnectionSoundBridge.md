...
### crossesWatchedConnection(t, mapId, systems, connections): boolean
Whether a jump (`fromSystemId` / `toSystemId`) resolves through `resolveTraversalEdges` to at least one connection this device watches on `mapId`. Shared with `PresenceSoundBridge`.
...
- The cue fires when `crossesWatchedConnection` holds, once per jump however many parallel holes match.
