...
- A successful sound save is announced to the device's other tabs (`announceSoundPrefs`); a tab that receives one adopts the blob as its on-screen and committed prefs (and so the engine's) and calls `router.refresh()`, so its map re-mounts the cue bridges against the new prefs without a reload.
...
- `useRouter().refresh()` — on a prefs save received from another tab
