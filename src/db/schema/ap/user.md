...
- `sound_prefs` (`soundPrefs`) — nullable `jsonb`, `.$type<SoundPrefs>()` (migration `0071`). The account's audio-cue preferences: master switch, volume, and an enabled flag + chosen sound id per cue (`pilotArrived`, `pilotLeft`, `watchedJump`, `killInSystem`, `rallySet`, `systemPinged`). NULL ⇒ `DEFAULT_SOUND_PREFS` (everything off). Read via `getSoundPrefs` (`session.ts`), which runs the column through `resolveSoundPrefs` so a partial or stale blob still yields usable prefs; written by `setSoundPrefsAction` (`actions/account.ts`) after Zod validation at the boundary (`src/lib/sounds/prefs.ts`).
