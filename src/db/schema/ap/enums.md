...
### overlayFitOverflow
`pgEnum('overlay_fit_overflow', ['proportional', 'grow_window', 'eat_pilot', 'eat_name', 'eat_type'])` — instance-wide policy on `ap_instance.overlay_fit_overflow` (default `proportional`, migration 0071) for the system overlay's "fit columns to content" action when the fitted widths exceed the overlay window. `proportional` takes the overrun from every resizable column in proportion to its fitted width; `grow_window` widens the Document PiP window instead; the three `eat_*` values make one named column absorb the whole overrun. No column shrinks past its floor, so residue left by an `eat_*` column that hits the floor falls back to a proportional share. Applied client-side by `fitOverlayColumns` ([[overlayColumnFit]]).
