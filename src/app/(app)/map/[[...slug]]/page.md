...
- Precomputes 24h stats (`statsForSystems`), read-side intel (`intelForSystems`), manual structure intel (`structuresForSystems`, scoped to the rows the viewing character's corp/alliance/private scope admits, and empty entirely for a viewer outside the map's owning entity), the viewer's structure-intel tenancy on this map (`requireIntelTenant` → `structureIntelEnabled`), the map's editable settings (`loadMapSettings`, for the settings dialog), the viewer's connection-travel-animation toggle (`getConnectionTravelAnimation`), the viewer's resolved signature-indicator prefs (`getSignatureIndicatorPrefs` — threshold + toggles for the stale/unscanned node indicators), the account roster (`getAccountCharacters`), the account's route-planner config (`loadRouteConfig` — settings + saved destinations, routes-module), the main character id (`getMainCharacterId`), the viewer's derived management authority (`canManageMap` → `canManage`, which reveals only the manager-only Roles & Permissions tab), the viewer's delegated capability set (`resolveMapCapabilities` → `capabilities`, which reveals every other management surface — behavior, auto-tagging, webhooks, share links, import/export, audit log), and the map's live public share links (`loadLiveShareBadges` → `liveShares`, the seed for the canvas's share indicator) for all visible systems in parallel and passes them to the client canvas. Initial pilot-presence (`MapViewData.presence`) ships with the map payload from `loadMapForView`.
...
### Depends On
- `@/lib/map/loadMap`, `@/lib/map/share` (`loadLiveShareBadges`), `@/lib/auth/rights` (`canManageMap`, `resolveMapCapabilities`), `@/lib/map/routeConfig`, `@/lib/map/stats`, `@/lib/map/intel`, `@/lib/structures/read`, `@/lib/structures/guard` (`requireIntelTenant`), `@/lib/session` (`getAccountCharacters`, `getConnectionTravelAnimation`, `getMainCharacterId`, `getMapLayout`, `getSignatureIndicatorPrefs`, `requireSession`), `@/components/map/MapCanvas`, `@/components/ui/card`. Behavior toggles, auto-tagging, webhooks, and the audit log live in-place on the canvas, each gated by its own entry in `capabilities`.
