...
**Mode:** Execute
**Status:** blocked — repo-wide sweep for pre-R4 gate-name prose (`canManageMap`/`map_update`) still incomplete after 2 repair passes; a 3rd review pass found more stale hits outside this stage's `Touches` (`src/app/(app)/map/[[...slug]]/page.md:12,17`, `src/app/api/map/README.md:17`, `src/app/api/map/utils.md:19`) — resume by fixing those three and re-reviewing
...
- **Stage 5** — Found and left an unrelated pre-existing bug while running the DB test suite to verify the two new tests: `tests/integration/permissions.test.ts`'s `resolveMapCapabilities: manager gets all, holder gets the union of grants` test asserts `managerCaps.size` is `7`, but `mapCapability` has carried 8 values since migration `0061` added `share_manage` (PR #226, unrelated to this plan) — the assertion was never updated. Confirmed via `git log -S` that the `7` was written in the original R4 PR (#217), before `share_manage` existed. Not fixed here (out of Stage 5's scope, unrelated to Findings 9/10 or the two assigned tests); the two tests this stage added both pass (33/34 in the file, the lone failure being this pre-existing one).
