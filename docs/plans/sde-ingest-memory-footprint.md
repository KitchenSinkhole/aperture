...
**Touches:** `src/lib/sde/ingest.ts`, `src/lib/sde/ingest.md`

Rather than building keep-sets from `parsed` after the fact, stage each table's new ids into an `UNLOGGED` table during the write phase (the rows are already flowing through, so it costs nothing) and let the sync be `DELETE ... WHERE NOT EXISTS`, keeping the existing per-table FK guards as additional `NOT EXISTS` clauses. Faster as well as smaller.

The ordering constraints in `ingest.md` still bind: the sync runs after the write phase and after `universe_wormhole` / `universe_system_static` are reseeded, before `computeHubProximity`, leaf-first through `DELETION_SPECS` so retention stays transitive. `syncSdeDeletions`' empty-keep-set guard needs an equivalent on the staged tables.
