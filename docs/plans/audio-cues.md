...
**Touches:** `public/sounds/voice/*.ogg` (the assets), `src/lib/sounds/prefs.ts` (+ `.md`): the `voice-*` ids must join `BUILT_IN_SOUND_IDS`, or `soundPrefsSchema` rejects any save naming one and `resolveSoundPrefs` rewrites a stored voice id back to the event's chime; `src/lib/sounds/catalog.ts` (+ `.md`): file-backed catalog entries with `{ id, label, event, files: Record<variant, path> }`, lazily fetched and decoded on first use, prefetched when selected in prefs; `src/lib/sounds/engine.ts` (+ `.md`): `BuiltInSound.synth` is synchronous and `SoundBackend.has` is synchronous, so a file-backed entry needs both widened — and a prefetch on `setPrefs`, since an unfetched voice file reports `has() === false` and silently falls back to the chime; the picker lists voice entries under a group heading.
