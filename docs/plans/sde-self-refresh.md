...
**Done when:** With `ap_sde_state` manipulated to each state, the banner shows: nothing when current; "out of date" only when behind latest *and* past `SDE_STALE_GRACE_HOURS` (a build FC published an hour ago is not an incident); "could not be updated" whenever the last refresh attempt failed and the instance is still behind — both messages in the same slot. Banner copy contains no em-dashes. `/setup` shows the detail and the manual refresh works.
