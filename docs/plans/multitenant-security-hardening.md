...
- **Neither the entity name nor the entity id reaches the client today.** `MapViewData.map` carries `type` but no owner id or name, and `StructureIntel` carries neither until Stage 5 adds the scope columns to it. So "naming the entity" needs a name resolved from somewhere — either widen the map view-model / read module to carry it, or render the bare tier ("Corp", "Alliance") and drop the entity name from the requirement. Settle that before writing the chip; it decides whether this stage touches server modules at all.
