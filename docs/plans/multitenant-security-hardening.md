...
- **Intel is a tenant-only feature of the map** (`requireIntelTenant`). The viewer-follows rule applies only on a map whose owning entity admits the viewer. A guest — a character `hasRoleAccess` admits from outside that entity — gets the whole surface closed on that map: no rows read, no create, an inert panel. Their own organisation's rows are withheld too, since serving them would overlay one org's intel on another's chain, and a create would otherwise stamp the host's scope onto a row the writer could never read back. Admins are exempt, as everywhere else.
...
**Spec:**
- Each row renders its scope (private / corp / alliance). Keep it visually distinct from the citadel's in-game owner corp, which the same row already displays — two "corp" facts side by side is the confusion this stage exists to prevent.
...
- **The chip names the tier, not the entity** — settled here, taking the spec's second option. Stage 5 put `scope` and `scopeEntityId` on `StructureIntel` but no *name* reaches the client, and resolving one server-side buys nothing: `structureVisibleTo` admits a non-admin only to rows scoped to their own character, corp or alliance, so for every ordinary viewer the tier already identifies the entity uniquely. Rendering the name would repeat the tier while putting a second corporation name beside the citadel owner's, which is the confusion the clause above exists to prevent. Only an admin, who reads across tenants, loses information; that is an ops surface, not the charting flow. The stage therefore touches no server module. See **Notes**.
- The dialog's "who will see this" line has a simpler source than the chip: scope derives from the current map, and the canvas already holds `data.map.type`, which maps one-for-one onto the three tiers. In edit mode it reads the row's own `scope` instead, which need not match the open map.
...
- The read-side unit is `structuresForSystems(systemIds, viewerCharacterId)` (`src/lib/structures/read.ts`). Its filter is `structureVisibleTo(viewer)` from `guard.ts`, the SQL form of the exported `scopeAdmits`; both are exported, so a cheap extra assertion is that they agree over the fixture set — that is the drift this split invites. **Assert the composed rule, `viewer.isAdmin || scopeAdmits(row, viewer)`, not `scopeAdmits` alone:** `scopeAdmits` deliberately carries no admin branch (both callers short-circuit on admin before reaching it) while `structureVisibleTo` admits every row for an admin, so a literal row-for-row comparison diverges for an admin viewer. Adding admin to `scopeAdmits` to make that comparison pass would silently widen the write gate and is the wrong repair. Also assert a viewer whose character is missing or not `active` reads `{}`.
- `POST /api/structures` treats the body's `mapId` as a scope selector only — it never checks the body's `systemId` is on that map. Record the accept-or-reject decision explicitly in the matrix rather than leaving it untested.
...
- Suite caveats: the dev DB is non-pristine, so snapshot and restore; triage failures in isolation because the full suite is parallel-flaky. Claim a fresh universe fixture id range and grep `tests/integration/*.test.ts` first — 98040xxx through 98046xxx are taken (this stage took 98047xxx, and 99060xxx for its org/character ids).
...
- **Stage 9** — as a character holding a role grant on another corp's map: open that map and confirm the Structures panel explains it is unavailable rather than listing anything, then open your own corp's map and confirm your corp's structures are all still there. The second half is what catches a tenancy gate that closed too wide.
- **Stage 8** — two characters in different corps, each on their own corp map, both looking at the same system: confirm each sees only their own corp's structures, that every row's scope chip shows the right tier, that the citadel's in-game owner corp is still shown and still distinguishable from the scope chip, and that the add dialog names the right audience before submit.
- **Stage 8** — open the add dialog from a **private** map and from a **corp** map and confirm the banner changes accordingly; then edit a corp-scoped row while standing on a private map and confirm the banner reports **corp**, not private. That mismatch is the one case where the map and the row disagree, and it is the case a writer would most easily misread.
...
**Tenancy gate** — notes take `requireIntelTenant` as well. It is map-level and table-agnostic, so the note read/create paths call the same guard: reads return nothing and creates are refused with 403 for a guest on the map, exactly as for structures.
...
**UI** — each note renders its scope, and the add/edit dialog names the audience **before** submit. The failure mode is a user writing staging intel believing it is private. No scope picker: scope derives from the map. Reuse `IntelScopeChip` (`src/components/sidebar/IntelScopeChip.tsx`), which takes a bare `IntelScope` and is table-agnostic; its `intelScopeAudience(scope)` export supplies the edit-mode audience sentence. The chip names the tier and not the entity — see Stage 8 for why, and keep the note panel consistent with it. The notes dialog needs the open map's `MapType` threaded in for its add-mode line, the way `StructureModule` takes `mapType`.

**Tests** — mirror Stage 9 for notes: two orgs, read omits the other's rows, cross-scope PATCH/DELETE 404s and writes nothing, create against a non-viewable map is rejected, same-scope CRUD unchanged, locked in-scope row still 409s. Add the one assertion structures do not need: a **capped** browser result page contains only admitted rows. The note-side describe block drops in beside the structure one in `tests/integration/cross-scope-intel.test.ts`, whose route-shaped `createAsRoute` / `patchAsRoute` / `deleteAsRoute` helpers and org fixtures are the shape to copy. Two things to carry across: the predicate/SQL agreement assertion must compose admin in (`viewer.isAdmin || noteScopeAdmits(...)`) for the reason Stage 9 records, and the note twin of `scopeAdmits` must not grow an admin branch to paper over it.
...
- **Stage 8** — the open name question was settled by reading `structureVisibleTo`, not by weighing render cost. Its non-admin branches match `scope_character_id = viewer`, `scope_corporation_id = viewer.corporationId`, `scope_alliance_id = viewer.allianceId`, so the set of scope entities an ordinary viewer can ever observe has exactly one member per tier — their own. A name is therefore a function of the tier for every viewer but an admin, and `scopeEntityId` stays on `StructureIntel` unread by the UI. Should a future surface need real names (an admin cross-tenant view), the join is three left joins in `read.ts` against `ap_character.name` / `ap_corporation.name` / `ap_alliance.name`, all of which `syncCharacterAuthz` keeps populated for any org that owns a map.
- **Stage 8** — the audience wording is deliberately row-relative ("the corporation it belongs to"), not viewer-relative ("your corporation"). An admin reads rows from organisations they are not in, and the guest path onto someone else's corp map means even a non-admin writer can produce a row scoped to a corp that is not theirs (Stage 4's note on `intelScopeForMap` taking the map's owner rather than the writer's affiliation). Viewer-relative copy would be wrong in exactly the cases that matter.
- **Stage 8** — the dialog's banner has two sources, not one: add mode reads the open map's `type`, edit mode reads `initial.scope`. They disagree whenever a row is edited from a map other than the one it was written on, which is routine given that visibility follows the viewer. Using `mapType` for both would have made the dialog assert the wrong audience on every cross-map edit.
- **Stage 9** — the `POST /api/structures` `systemId`-not-on-`mapId` question was resolved as **accepted, pinned by a test**. `mapId` is a scope selector and structures are system-scoped by design (a row surfaces on every map showing its system), so the reachable outcome is "write intel about an arbitrary system into a scope you could reach anyway by adding that system to your own map first". That is not a cross-tenant capability, and requiring the system to be on the map would break the legitimate flow of logging a structure in a system you have not charted yet. The test asserts the acceptance so a future change to it is deliberate.
- **Stage 9** — the erased-owner fixture (`scope='private'`, `scope_character_id` NULL) has to be inserted directly. No write path can produce it: `intelScopeForMap` never returns a `private` scope with a null character, and reaching it through the real path would mean deleting a character mid-suite.
- **Stage 9** — the DELETE probe used to verify regression value doubles as evidence the "writes nothing" clause is load-bearing: with the `scopeAdmits` check stubbed out of `requireStructureMutate`, the cross-scope DELETE actually removed the victim's row, and three *later* read-side assertions failed as collateral. A guard test that only checked the status code would have reported one failure instead of four.
- **Stage 9** — the static route-rights scan (`tests/unit/route-rights-coverage.test.ts`) walks `src/app/api/map/**` only, so `/api/structures/**` is covered by nothing static: a new structure route landing without `requireStructureMutate` would be caught only by review. Left alone (outside this stage's Touches), but it is the cheapest remaining hardening in this area — either widen the scan's root or give it a second root with a `requireStructureMutate`/`requireMapView` expectation.
...
- **Review of PR #245** — the read filter keyed on the viewer alone left the guest path incoherent in both directions: a role-granted viewer saw none of the host map's structures but did see their own corp's rows overlaid on someone else's chain, and a create from that map succeeded, stamped the host's scope, and vanished on reload with no way to edit or delete it (`requireStructureMutate` answers 404). Both were the same missing gate. `requireIntelTenant` closes the feature on a map the viewer's entity does not own, and `structuresForSystems` now requires the map id so no caller can read without it. Stage 4's note above — a guest writing on someone else's private map cannot see their own row afterwards — is subsumed: they can no longer write it.
